package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1583:String = "asset-libraries";
      
      private static const const_1577:String = "library";
      
      private static const const_1581:String = "service-libraries";
      
      private static const const_1578:String = "library";
      
      private static const const_1579:String = "component-libraries";
      
      private static const const_1576:String = "library";
      
      private static const const_1582:String = "error_data";
      
      private static const const_1575:String = "error_cat";
      
      private static const const_1580:String = "error_desc";
       
      
      private var var_383:LibraryLoaderQueue;
      
      private var var_822:IEventDispatcher;
      
      private var var_556:uint;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            ...
            case Core.const_778:
               ...
               var_430.addChild(new ProfilerViewer(this.var_219));
               break;
            case Core.const_625:
      ...
      {
         return this.var_556 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_280[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1582,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1575,String(param3));
         ErrorReportStorage.setParameter(const_1580,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_822 = loadingEventDelegate;
         if(this.var_383 == null)
         {
         ...
         try
         {
            node = config.child(const_1583)[0];
            if(node != null)
            {
               list = node.child(const_1577);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_229,e);
         }
         try
         {
            node = config.child(const_1581)[0];
            if(node != null)
            {
               list = node.child(const_1578);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_229,e);
         }
         try
         {
            node = config.child(const_1579)[0];
            if(node != null)
            {
               list = node.child(const_1576);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_229,e);
         }
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_822)
         {
            this.var_822.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_822)
         {
            ...
            this.var_822.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_822)
                  {
                     this.var_822.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_556 - this.var_383.length,this.var_556,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_219 ? 0 : this.var_1564 & 0;
         if(_loc3_ == Core.const_625)
         {
            this.var_280[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_280[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc4_:int = !!this.var_219 ? 0 : this.var_1564 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_107)
         {
            _loc3_ = this.var_280[_loc5_] as Array;
            if(_loc4_ == Core.const_625)
            {
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_677[priority] = 0;
            receivers = this.var_280[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var maxTimePerFrame:uint = 1000 / var_430.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_107)
         {
            t = getTimer();
            ...
            {
               if(this.var_677[priority] < priority)
               {
                  ++this.var_677[priority];
                  skip = true;
               }
            }
            if(!skip)
