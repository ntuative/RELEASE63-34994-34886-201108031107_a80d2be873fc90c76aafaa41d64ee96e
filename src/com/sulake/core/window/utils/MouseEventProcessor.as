package com.sulake.core.window.utils
{
   ...
   public class MouseEventProcessor implements IEventProcessor, IDisposable
   {
      ...
      protected static var var_438:Point = new Point();
       
      
      protected var var_761:Point;
      
      protected var var_144:WindowController;
      
      protected var var_71:WindowController;
      
      protected var var_185:IWindowRenderer;
      
      protected var var_94:IDesktopWindow;
      
      protected var var_948:IWindowContextStateListener;
      ...
      {
         super();
         this.var_761 = new Point();
         if(var_364 == null)
         {
            var_364 = new Array();
            var_364[0] = MouseCursorType.const_334;
            var_364[1] = MouseCursorType.const_30;
            var_364[2] = MouseCursorType.const_334;
            var_364[3] = MouseCursorType.const_334;
            var_364[4] = MouseCursorType.const_334;
            var_364[5] = MouseCursorType.const_30;
            var_364[6] = MouseCursorType.const_334;
         }
         if(var_316 == null)
         {
            var_316 = new Array();
            var_316[0] = WindowState.const_91;
            var_316[1] = WindowState.const_92;
            var_316[2] = WindowState.const_89;
            var_316[3] = WindowState.const_72;
            var_316[4] = WindowState.const_57;
            var_316[5] = WindowState.const_95;
            var_316[6] = WindowState.const_94;
         }
      }
      
      public static function setMouseCursorByState(param1:uint, param2:uint) : void
      {
         var _loc3_:int = var_316.indexOf(param1);
         if(_loc3_ > -1)
         {
            var_364[_loc3_] = param2;
         }
      }
      ...
      protected static function convertMouseEventType(param1:MouseEvent, param2:IWindow, param3:IWindow) : WindowMouseEvent
      {
         ...
         switch(param1.type)
         {
            ...
            case MouseEvent.MOUSE_UP:
               ...
               _loc4_ = !!_loc6_ ? "null" : WindowMouseEvent.const_277;
               break;
      ...
      public function process(param1:EventProcessorState, param2:IEventQueue) : void
      {
         ...
         this.var_94 = state.desktop;
         this.var_71 = state.var_1422 as WindowController;
         this.var_144 = state.var_1420 as WindowController;
         this.var_185 = state.renderer;
         this.var_948 = state.var_1423;
         eventQueue.begin();
         this.var_761.x = -1;
         this.var_761.y = -1;
         ...
         while(true)
         {
            ...
            if(event.stageX != this.var_761.x || event.stageY != this.var_761.y)
            {
               this.var_761.x = event.stageX;
               this.var_761.y = event.stageY;
               array = new Array();
               this.var_94.groupParameterFilteredChildrenUnderPoint(this.var_761,array,WindowParam.const_29);
            }
            ...
            if(index == 0)
            {
               ...
               {
                  case MouseEvent.MOUSE_MOVE:
                     if(this.var_71 != this.var_94 && !this.var_71.disposed)
                     {
                        this.var_71.getGlobalPosition(var_438);
                        ...
                        this.var_71 = WindowController(this.var_94);
                        tempWindowEvent.recycle();
                     }
                     break;
                  case MouseEvent.MOUSE_DOWN:
                     window = this.var_94.getActiveWindow();
                     if(window)
                     {
                        window.deactivate();
                     }
               ...
               if(child != null && child.visible)
               {
                  if(event.type == MouseEvent.MOUSE_MOVE)
                  {
                     if(child != this.var_71)
                     {
                        ...
                        if(!child.disposed)
                        {
                           child.getGlobalPosition(var_438);
                  ...
                  else if(event.type == MouseEvent.MOUSE_UP || event.type == MouseEvent.CLICK || event.type == MouseEvent.MOUSE_DOWN)
                  {
                     if(this.var_71 && !this.var_71.disposed)
                     {
                        ...
                        {
                           this.var_948.mouseEventReceived(event.type,this.var_71);
                        }
                     }
                  }
                  temp = child.parent;
         ...
         state.desktop = this.var_94;
         state.var_1422 = this.var_71;
         state.var_1420 = this.var_144;
         state.renderer = this.var_185;
         state.var_1423 = this.var_948;
      }
      ...
      private function passMouseEvent(param1:WindowController, param2:MouseEvent, param3:Boolean = false) : WindowController
      {
         ...
         if(param2.type == MouseEvent.MOUSE_UP)
         {
            ...
            {
               if(this.var_144 && !this.var_144.disposed)
               {
                  this.var_144.update(this.var_144,convertMouseEventType(new MouseEvent(MouseEvent.MOUSE_UP,false,true,param2.localX,param2.localY,null,param2.ctrlKey,param2.altKey,param2.shiftKey,param2.buttonDown,param2.delta),this.var_144,param1));
                  this.var_144 = null;
                  if(param1.disposed)
                  {
                     return null;
         ...
         if(!_loc4_)
         {
            _loc9_ = this.var_185.getDrawBufferForRenderable(param1);
            if(!param1.validateLocalPointIntersection(_loc5_,_loc9_))
            {
               return null;
            }
         }
         if(param1.testParamFlag(WindowParam.const_522))
         {
