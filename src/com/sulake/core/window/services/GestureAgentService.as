package com.sulake.core.window.services
{
   ...
   public class GestureAgentService implements IGestureAgentService, IDisposable
   {
      ...
      protected var var_1564:uint = 0;
      
      protected var var_313:Function;
      
      protected var var_1434:int;
      
      protected var var_1433:int;
      ...
      public function begin(param1:IWindow, param2:Function, param3:uint, param4:int, param5:int) : IWindow
      {
         ...
         {
            this._window = param1;
            this._window.addEventListener(WindowEvent.const_339,this.clientWindowDestroyed);
            this.var_313 = param2;
            this._working = true;
            this.var_1434 = param4;
            this.var_1433 = param5;
      ...
      protected function operate(param1:TimerEvent) : void
      {
         this.var_1434 *= 0.75;
         this.var_1433 *= 0.75;
         if(Math.abs(this.var_1434) <= 1 && Math.abs(this.var_1433) <= 1)
         {
            this.end(this._window);
         }
         else if(this.var_313 != null)
         {
            this.var_313(this.var_1434,this.var_1433);
         }
      }
      ...
      public function end(param1:IWindow) : IWindow
      {
         ...
         if(this._working)
         {
            if(this._window == param1)
            {
               ...
               {
                  this._window.removeEventListener(WindowEvent.const_339,this.clientWindowDestroyed);
               }
               this._window = null;
               this._working = false;
            }
