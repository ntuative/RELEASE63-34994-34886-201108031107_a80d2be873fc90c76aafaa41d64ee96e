package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1860:int = 4;
      ...
      {
         return this.var_490.length + this.var_312.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_312.length < const_1860 && this.var_490.length > 0)
            {
               _loc1_ = this.var_490.shift();
               this.var_312.push(_loc1_);
               _loc1_.resume();
            }
         }
      }
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_490.indexOf(loader);
            if(index > -1)
            {
               this.var_490.splice(index,1);
            }
            index = this.var_312.indexOf(loader);
            if(index > -1)
            {
               this.var_312.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_490)
            {
               if(param2)
               {
