package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private var var_427:IBitmapWrapperWindow;
      ...
      private function createView() : void
      {
         ...
         this.var_427 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_754 && this.var_1711)
         {
            _loc5_ = this.var_27.manager.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_112,this._gender,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_42,parseInt(FigureData.const_752));
         ...
         if(this.var_427)
         {
            if(this.var_427.bitmap)
            {
               this.var_427.bitmap.dispose();
            }
            this.var_427.bitmap = new BitmapData(this.var_427.width,this.var_427.height,true,0);
            _loc7_ = (this.var_427.width - _loc1_.width) / 2;
            _loc8_ = (this.var_427.height - _loc1_.height) / 2;
            this.var_427.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            case "set_button":
               this.var_754 = this.var_27.figureData.getFigureString();
               this._gender = this.var_27.gender;
               this.var_27.handler.saveWardrobeOutfit(this.var_2216,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_754)
               {
                  this.var_27.loadAvatarInEditor(this.var_754,this._gender,this.var_27.clubMemberLevel);
               }
         }
      }
