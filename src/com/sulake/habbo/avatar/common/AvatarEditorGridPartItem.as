package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_896:IFigurePartSet;
      ...
      private var var_734:Rectangle;
      
      private var var_2157:int = 0;
      
      private var var_497:IAvatarRenderManager;
      
      private var var_1155:Boolean;
      
      private var var_2155:BitmapData;
      
      private const const_2402:int = 13421772;
      
      private const const_2401:int = 16777215;
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this._colors = param4;
         this.var_2156 = param5;
         if(param3 == null)
         {
            this.var_760 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_2157 = Math.max(this.var_2157,_loc6_.colorLayerIndex);
            }
         }
         this.var_497 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_760 != null && !this.var_2156)
            {
               _loc3_ = this.var_760;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_896)
            {
               switch(this.var_896.clubLevel)
               {
                  case HabboClubLevelEnum.const_53:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_33:
                     _loc2_.style = HabboIconType.const_792;
                     break;
                  case HabboClubLevelEnum.const_36:
                     _loc2_.style = HabboIconType.const_1030;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1664.color = this.const_2402;
         }
         else
         {
            this.var_1664.color = this.const_2401;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_497.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_497.isFigureReady(_loc1_))
         {
            this.var_497.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_392[_loc6_] + "_" + FigureData.const_149;
               _loc4_ = this.var_497.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_734 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_2155)
               {
                  ...
                  this.var_2155 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_734.width,this.var_734.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_734.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_734.y;
               if(this.var_2156 && _loc2_.colorLayerIndex > 0)
               {
