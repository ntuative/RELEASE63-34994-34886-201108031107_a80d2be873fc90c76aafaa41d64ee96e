package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1138:int = 3;
      
      private var var_505:AssetLoaderStruct;
      ...
      private const const_2406:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_505 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_1140 = new Timer(this.const_2406,1);
         this.var_1140.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_505)
         {
            this.var_505.addEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_505.addEventListener(AssetLoaderEvent.const_40,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_505)
         {
            this.var_505.removeEventListener(AssetLoaderEvent.ASSET_LOADER_EVENT_COMPLETE,this.onConfigurationComplete);
            this.var_505.removeEventListener(AssetLoaderEvent.const_40,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         if(this.var_1138 <= 0)
         {
            HabboWebTools.logEventLog("Figuremap download error " + this.var_1397);
            Logger.log("AVATAR ASSET ERROR: Could not download figuremap.xml.");
         }
         else
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_1138);
            if(this.var_1397.indexOf("?") > 0)
            {
               _loc2_ = this.var_1397 + "&retry=" + this.var_1138;
            }
            else
            {
               _loc2_ = this.var_1397 + "?retry=" + this.var_1138;
            ...
            this.var_505 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_3040,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1395[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1395[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_2201)
         {
            this.var_1396.push([param1,param2]);
            return;
         }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_1139)
         {
            _loc7_ = true;
            _loc4_ = this.var_1139[_loc3_];
         ...
         {
            delete this.var_1139[_loc3_];
         }
         while(_loc6_ < this.var_928.length)
         {
            _loc5_ = this.var_928[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_928.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_1140.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_927.length > 0 && this.var_928.length < this.const_1860)
         {
            _loc1_ = this.var_927[0];
            _loc1_.startDownloading();
            this.var_928.push(this.var_927.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_927.indexOf(param1) == -1 && this.var_928.indexOf(param1) == -1)
         {
            this.var_927.push(param1);
            this.processPending();
         }
      }
