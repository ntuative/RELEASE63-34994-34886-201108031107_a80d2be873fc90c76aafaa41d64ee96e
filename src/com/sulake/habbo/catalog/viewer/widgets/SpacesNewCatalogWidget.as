package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class SpacesNewCatalogWidget extends ItemGridCatalogWidget implements IDisposable, ICatalogWidget, IItemGrid
   {
      ...
      private var var_459:ISelectorWindow;
      ...
      public function SpacesNewCatalogWidget(param1:IWindowContainer, param2:ISessionDataManager)
      {
         ...
         this.var_641 = [0,0,0];
         this.var_117 = ["group.walls","group.floors","group.views"];
         super(param1,param2);
         this.var_459 = _window.findChildByName("groups") as ISelectorWindow;
         if(this.var_459)
         {
            _loc3_ = 0;
            while(_loc3_ < this.var_459.numSelectables)
            {
               _loc4_ = this.var_459.getSelectableAt(_loc3_);
      ...
      override public function dispose() : void
      {
         ...
         var _loc2_:* = null;
         super.dispose();
         for each(_loc1_ in this.var_214)
         {
      ...
      override public function init() : Boolean
      {
         ...
         this.switchCategory(this.var_117[this.var_533]);
      ...
      {
         var _loc2_:int = this.var_641[this.var_533];
         var _loc3_:Offer = this.var_214[this.var_533][_loc2_];
      ...
      {
         var _loc2_:* = null;
         if(param1 > -1 && param1 < var_520.numGridItems)
         {
            _loc2_ = this.var_214[this.var_533][param1];
      ...
      override public function select(param1:IGridItem) : void
      {
         ...
         this.var_641[this.var_533] = (this.var_214[this.var_533] as Array).indexOf(_loc2_);
         this.updateRoomPreview();
      }
      
      private function updateRoomPreview() : void
      {
         var _loc1_:int = this.var_641[0];
         var _loc2_:int = this.var_641[1];
         var _loc3_:int = this.var_641[2];
         var _loc4_:Offer = this.var_214[0][_loc1_];
         var _loc5_:Offer = this.var_214[1][_loc2_];
         var _loc6_:Offer = this.var_214[2][_loc3_];
      ...
      private function createOfferGroups() : Boolean
      {
         ...
         {
            if(!(_loc1_.pricingModel == Offer.const_396 || _loc1_.pricingModel == Offer.const_411))
            {
               continue;
            }
            ...
            if(!(_loc2_.productType == ProductTypeEnum.const_65 || _loc2_.productType == ProductTypeEnum.const_78))
            {
               continue;
            }
            ...
            while(this.var_214.length < this._groupNames.length)
            {
               this.var_214.push(new Array());
            }
            switch(_loc4_)
            {
               case "floor":
                  (this.var_214[_loc5_] as Array).push(_loc1_);
                  break;
               case "wallpaper":
                  (this.var_214[_loc5_] as Array).push(_loc1_);
                  break;
      ...
      private function onSelectGroup(param1:WindowEvent) : void
      {
         ...
         if(_loc2_)
         {
            _loc3_ = this.var_459.getSelectableIndex(_loc2_);
      ...
      private function switchCategory(param1:String) : void
      {
         ...
         this.var_459.setSelected(this.var_459.getSelectableByName(param1));
         var _loc2_:int = -1;
         switch(param1)
         {
            case "group.walls":
         ...
         if(_loc2_ > -1)
         {
            ...
            if(var_520)
            {
               var_520.destroyGridItems();
            }
            page.replaceOffers(this.var_214[this.var_533],false);
            resetTimer();
            populateItemGrid();
            ...
            _loc3_ = this.var_641[this.var_533];
