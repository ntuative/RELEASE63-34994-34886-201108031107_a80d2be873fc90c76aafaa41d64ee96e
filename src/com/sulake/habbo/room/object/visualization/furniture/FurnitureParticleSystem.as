package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_113:FurnitureParticleSystemEmitter;
      
      private var var_98:BitmapData;
      
      private var var_287:IRoomObjectSprite;
      ...
      private var var_1314:Number = 1.0;
      
      private var var_704:BitmapData;
      
      private var var_854:ColorTransform;
      
      private var var_1606:ColorTransform;
      
      private var var_2047:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_778:Number = 1.0;
      
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         super();
         this.var_1315 = new Dictionary();
         this._visualization = param1;
         this.var_854 = new ColorTransform();
         this.var_854.alphaMultiplier = 1;
         this.var_1606 = new ColorTransform();
         this.var_2047 = new Matrix();
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_1315)
         {
            _loc1_.dispose();
         }
         this.var_1315 = null;
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_113 = this.var_1315[param1];
         this.var_1058 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_2048 >= 0)
         {
            this.var_287 = this._visualization.getSprite(this.var_2048);
            if(this.var_287 && this.var_287.asset)
            {
               if(this.var_287.width <= 1 || this.var_287.height <= 1)
               {
                  return;
               }
               if(this.var_98 == null)
               {
                  this.var_98 = this.var_287.asset.clone();
                  if(this.var_854.alphaMultiplier != 1)
                  {
                     this.var_704 = new BitmapData(this.var_98.width,this.var_98.height,true,4278190080);
                  }
               }
               this.var_2505 = -this.var_287.offsetX;
               this.var_2506 = -this.var_287.offsetY;
               this.var_287.asset = this.var_98;
            }
            if(this.var_98)
            {
            ...
            if(this.var_704)
            {
               this.var_704.fillRect(this.var_704.rect,4278190080);
            }
         }
      }
      ...
      {
         if(this.var_113 && this.var_113.roomObjectSpriteId == param3)
         {
            return this.var_113.y * this.var_1314;
         }
         return 0;
      }
      ...
      public function controlsSprite(param1:int) : Boolean
      {
         if(this.var_113)
         {
            return this.var_113.roomObjectSpriteId == param1;
         }
         return false;
      }
      ...
      {
         if(!this.var_113 || !this.var_287)
         {
            return;
         }
         if(this.var_98 && this.var_287.asset != this.var_98)
         {
            this.var_287.asset = this.var_98;
         }
         if(this.var_1058)
         {
            if(this.var_113.roomObjectSpriteId >= 0)
            {
               this._visualization.getSprite(this.var_113.roomObjectSpriteId).visible = false;
            }
         }
      }
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1058 && this.var_113.hasIgnited)
         {
            this.var_1058 = true;
         }
         _loc8_ = this.var_1201 * this.var_1314;
         this.var_113.update();
         if(this.var_1058)
         {
            if(this.var_113.roomObjectSpriteId >= 0)
            ...
            {
               this.var_98.draw(this.var_704,this.var_2047,this.var_854,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_113.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2505 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1314;
               _loc4_ = this.var_2506 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1314;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1606.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_98.draw(_loc7_,this._translationMatrix,this.var_1606,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2833 = parseInt(param1.@size);
         this.var_2048 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1201 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1314 = this.var_2833 / 64;
         this.var_778 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_778 = Math.min(this.var_778,1);
         this.var_854.alphaMultiplier = this.var_778;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc19_ = _loc23_.@is_emitter == "AnimationAction" ? false : true;
